...
\end{lem}
\begin{proof}
  We prove the descendant-side condition.
  \begin{align*}
    &q \cdot \CovDesc{2}(uv) - \left(\CovDesc{1}(uv)\right)^2\\
    &= q \cdot \sum_{i=1}^{q} \left(\id(x_i)\right)^2 - \left(\sum_{i=1}^{q} \id(x_i)\right)^2 \\
    &= \sum_{i=1}^{q} (q-1)\left(\id(x_i)\right)^2 -
    \sum_{i=1}^{q-1} \sum_{j=i+1}^{q} 2 \id(x_i) \cdot \id(x_j) \\
    &= \sum_{i=1}^{q-1} \sum_{j=i+1}^{q} \left(\id(x_i) - \id(x_j)\right)^2;
  \end{align*}
  the last equality holds by rearranging the terms.
  Since each term in the last summation is non-negative, 
  the entire summation is equal to zero if and only if
  $\id(x_i) = \id(x_j)$ for all $i,j \in \{1, \dots, q\}$,
  which is equivalent to $x_1 = x_2 = \cdots = x_q$.
  The ancestor-side condition can be proved similarly.
\end{proof}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    an initilal value $A(e)$ for all tree edges $e$ in $E(T)$,
...
\begin{proof}
  ...
  Since each update operation is processed in $O(1)$ time,
...
\begin{lem}
  ...
  \item Weighted cover information $\CovDesc{1}(uv)$ and $\CovDesc{2}(uv)$;
...
\begin{proof}
  ...
  \item For $\CovDesc{i}(uv)$, set $\alpha := \id(x)^i$ for $i \in \{1, 2\}$; and
  \item For $\CovAnc{i}(uv)$, set $\alpha := \id(y)^i$ for $i \in \{1, 2\}$.
  \end{itemize}
