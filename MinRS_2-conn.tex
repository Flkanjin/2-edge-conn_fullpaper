...
$
that is, the number of distinct descendants that serve as tails of back edges covering~$uv$.
...
We first perform a DFS on $G$ starting from an arbitrary vertex $r \in V$
and let $T$ be the resulting DFS-tree.
...
\begin{proof}
  ...
  Therefore, the only back edges that contribute a nonzero amount are those
  that cover $uv$,
...
\begin{lem}
  ...
  Let $G=(V,E)$ be a 2-edge-connected undirected graph with $|V|\geq 3$ and,
  $T$ be a DFS-tree of $G$ rooted at $r\in V$.
  Let $x$ be a leaf of $T$ that has back edges and let $y :=\lbeTarget(x)$.
...
\begin{proof}
  ...
  % \medskip
  ($\Longleftarrow$)
  We suffice to show that $G - x$ is 2-edge-connected.
  By \lemref{leaf_not_ap},
  $G - x$ is connected.
...
Here, we introduce the \emph{source-exclusive coverage set}~$\SrcExc_v$ of $v$ for each vertex $v \in V$,
...
By \lemref{leaf_mrs},
$\{v\}$ is a MinRS of $G$ if and only if $\SrcExc_v = \emptyset$.
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\SrcExc_l := \SrcExc_l \cup \{uv\}$ \textbf{if} $\CovSrc(uv) = 1$; \Comment{$l$ is not \Nil if this line is executed}
