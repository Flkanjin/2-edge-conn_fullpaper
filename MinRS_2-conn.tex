...
\newcommand{\CovDesc}[1]{\mathit{CovDesc}_{#1}}
\newcommand{\CovAnc}[1]{\mathit{CovAnc}_{#1}}

\newcommand{\maxTwoDegPath}{degree-two block\xspace}
\newcommand{\MaxTwoDegPath}{\MFUsentencecase{\maxTwoDegPath}\xspace}
\newcommand{\MaxTwoDegPathCap}{Degree-Two Block\xspace}
...
\]
We refer to these quantities as \emph{weighted cover information}.
...
% we establish several structural properties of weighted cover information.
...
we briefly illustrate how weighted cover information
...
using the weighted cover information.
%
...
\begin{proof}
  ...
  \end{align*}
  Since each term in the last summation is non-negative, 
...
and $\alpha$ is a value to be added to all tree edges.
The goal is to compute the final value stored in each tree edge
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    an initial value $A(e)$ for all tree edges $e$ in $E(T)$,
    and a sequence of update operations $(x,y,\alpha)$
    \Ensure The final values for all tree edges
    ...
    \EndFor;
    \State \textbf{output} $\mathit{value}(e)$ for all tree edges $e$ in $E(T)$
...
\begin{lem}
  ...
  for all tree edges in $O(q+n)$ time.
\end{lem}
\begin{proof}
  By \lemref{diff_agg},
  ...
  which means we can correctly compute the final values for all tree edges.
...
\begin{lem}
  ...
  \item Weighted cover information $\CovDesc{1}(uv)$ and $\CovDesc{2}(uv)$; and
  \item Weighted cover information $\CovAnc{1}(uv)$ and $\CovAnc{2}(uv)$.
  \end{itemize}
\end{lem}
\begin{proof}
...
have been studied in \cite{ISYH.2022,ITO.2022,TH.2025}.
In these works, they showed the following necessary condition for MinRSs.
\begin{lem}[Observation 3 in \cite{ISYH.2022,ITO.2022}(, Lemma 7 in \cite{TH.2025})]
  \label{lem:mrs_cond}
  Let $G=(V,E)$ be a 2-edge-connected undirected graph with $|V|\geq 3$.
  If a \nonempty proper subset $S \subset V$ is a MinRS of $G$,
  then,
  \begin{itemize}
  \item $S$ forms \maxTwoDegPath in $G$;
  or
...
\begin{proof}
  ...
  then it is either $\{r\}$ ($\deg_G(r) \geq 3$) or a \maxTwoDegPath~$P$ containing $r$ (otherwise).
...
for the sake of simplifying discussions in the following sections.
\begin{lem}
  ...
  that is an endopoint of any back edge
  appears in $P$; and
...
\begin{proof}
  ...
  if an internal vertex of $T$ is an endopoint of some back edge,
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \For{\textbf{each} leaf $l$ of $T$}
      ...
      \end{cases}$;
      \State $\mathit{is\_MinRS} := \True$;
      \For{\textbf{each} tree edge $(u,v)$ in $\mathit{covering\_sets}_l$}
        \State $\mathit{is\_MinRS} := \False$ \textbf{if} $u \not\in S$ and $v \not\in S$;
        \Comment{You can break the loop here}
        % \If{$u \not\in S$ and $v \not\in S$}
        %   \State $\mathit{is\_MinRS} := \False$
        %   \Comment{You can break the loop here}
        % \EndIf
      \EndFor;
      \State $\cupappend{\MY}{\{S\}}$ \textbf{if} $\mathit{is\_MinRS}$;
    \EndFor;
...
\begin{proof}
  ...
  we can compute the weighted cover information
...
\begin{proof}
  ...
  $\CovMult(e_P)$ in $G_P$ equals any $\CovMult(uv)$ in $G$
  for a tree edge $(u,v)$ on the $a$--$b$ path in $T$.
  No vertex in $P$ is an endopoint of any back edge by \lemref{max2degpath_props}(i).
